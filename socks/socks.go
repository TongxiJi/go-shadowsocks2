...
func Handshake(rw io.ReadWriter) (Addr, error) {
	...
	// write VER METHOD
	if _, err := rw.Write([]byte{SocksVersion, 0}); err != nil {
		return nil, err
	}
	...
	switch cmd {
	case CmdConnect:
		_, err = rw.Write([]byte{SocksVersion, 0, 0, 1, 0, 0, 0, 0, 0, 0}) // SOCKS v5, reply succeeded
	case CmdUDPAssociate:
		if !UDPEnabled {
			return nil, ErrCommandNotSupported
		}
		listenAddr := ParseAddr(rw.(net.Conn).LocalAddr().String())
		_, err = rw.Write(append([]byte{SocksVersion, 0, 0}, listenAddr...)) // SOCKS v5, reply succeeded
		if err != nil {
			return nil, ErrCommandNotSupported
		}
